# PC Sampling SSD Firmware Fuzzer v6.1

OpenOCD PCSR(PC Sampling Register) 방식으로 커버리지를 수집하고, `nvme-cli` subprocess를 통해 NVMe 명령어를 SSD에 전달하는 Coverage-Guided Fuzzer.

v6.1의 핵심: **libFuzzer 스타일 Rule-Based Schema Mutation** — 42개 NVMe 커맨드 CDW 필드를 타입별 전략(ENUM/LBA/FLAGS/SIZE_DW/…)으로 자동 변형 + **IO_ADMIN_RATIO=3** (I/O 명령 75% 우선) + **Calibration-only FormatNVM/Sanitize** (FTL 상태 리셋 후 퍼징 풀에서 제거).
...
```
PC_Sampling/
├── pc_sampling_fuzzer_v6.1.py   # 메인 퍼저 (내부 버전: v6.1)
...
**v6.1 해결**: 각 커맨드의 CDW 필드 스키마를 정의하고, 필드 타입에 따라 적절한 변형을 자동 적용.
...
| 버전 | 주요 변경 |
|------|-----------|
| **v6.1** | Rule-Based Schema Mutation (42커맨드/~150필드/8타입), IO_ADMIN_RATIO=3, Calibration-only FormatNVM+Sanitize, NSZE 캐싱, mutation_stats schema_field, 신규 커맨드 21개 추가 |
