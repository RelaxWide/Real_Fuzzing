# PC Sampling SSD Firmware Fuzzer v7.6

OpenOCD PCSR(PC Sampling Register) 비침습 샘플링 + `nvme-cli` passthru 기반 Coverage-Guided + State-Aware Fuzzer.

v7.6 핵심: **시각화 개선** — `coverage_growth.png`에 velocity bar + plateau 음영 + 마일스톤 annotation 추가. `firmware_map.png`를 BB-coverage 그라데이션 + 전체 함수 treemap + Top-N 라벨로 발전. `csfuzz_dynamics.png` 신규 (p 추이 + corpus 성장 + m1/m2 reward 비교). per-command CFG는 가독성 낮아 제거.

v7.5에서 도입된 SequenceSeed corpus / ctx 모드(full / lba_nlb) / 2-pass favored cull 그대로 유지.
