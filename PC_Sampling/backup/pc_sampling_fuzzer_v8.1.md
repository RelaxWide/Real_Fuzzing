# PC Sampling SSD Firmware Fuzzer v8.1

OpenOCD PCSR 비침습 샘플링 + `nvme-cli` passthru 기반 Coverage-Guided + State-Aware Fuzzer.

**v8.1 핵심: P9(Cortex-R5) 전용 J-Link(pylink) halt 샘플러 추가**
v8.0 의 P9 OpenOCD-telnet halt 샘플러가 desync 로 컨트롤러를 멈추게 하던 문제를, pylink 로
J-Link 를 직접 제어하는 `JLinkHaltSampler` 로 대체했다. **PM9M1/BM9H1(PCSR) 경로는 무변경.**

v8.0 제품 일반화(`PRODUCT_PROFILES`), v7.x 의 모든 기능은 그대로 유지. 제품 일반화/프로파일
필드 등 v8.0 상세는 `pc_sampling_fuzzer_v8.0.md` 참조 — 이 문서는 v8.1 변경분만 다룬다.
