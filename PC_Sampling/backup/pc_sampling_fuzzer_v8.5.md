# PC Sampling SSD Firmware Fuzzer v8.5

OpenOCD PCSR 비침습 샘플링 + `nvme-cli` passthru 기반 Coverage-Guided + State-Aware Fuzzer.

> v8.5 = v8.4 전체 기능 보존 + **PCSR always-on 계측**(백그라운드/명령 사이 total coverage 포착) +
> prefill 샘플링 통합 + `--ignore-opcodes` + `--no-erase`(전체 소거 차단) + always-on 도입으로 생긴
> static-coverage 일관성 버그 일괄 수정. `sampler_type!='pcsr'`(halt/jlink_halt/null)이면 동작 불변.

---

## v8.5 핵심: PCSR always-on 계측
