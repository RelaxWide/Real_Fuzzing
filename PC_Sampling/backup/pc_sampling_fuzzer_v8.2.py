#!/usr/bin/env python3
"""
PC Sampling 기반 SSD 펌웨어 Coverage-Guided Fuzzer v8.2
...
R8_DPIDR_VALS = (0x6ba02476, 0x6ba02477,   # SWD DPIDR
                 0x6ba00476, 0x6ba00477)   # JTAG IDCODE/DPIDR
...
PRODUCT_PROFILES = {
    'PM9M1': {
        'interface':         'swd',
        'openocd_config':     OPENOCD_CONFIG,        # r8_pcsr.cfg
    ...
    'BM9H1': {
        'interface':         'jtag',
        'openocd_config':     OPENOCD_CONFIG_JTAG,    # r8_pcsr_jtag.cfg
        'jlink_device':       JLINK_DEVICE,           # Cortex-R8
        'tcl_prefix':         'r8',
    ...
    # ── P9: Cortex-R5 r1p3, SWD ── [v8.0~v8.2]
    # J-Link 로 확정된 것: SW-DP 0x6BA02477, AP[0]=APB-AP 단일, 0x80030000 = Cortex-R5 디버그
    #   base(PID 0xC15), 단일코어. DBGPCSR 미구현 → v8.1 부터 pylink 직접 halt(JLinkHaltSampler).
    # v8.2: OpenOCD/PCSR/UFAS 전용 키(openocd_config/tcl_prefix/pcsr_addrs/power_addr/power_mask/
    #   ufas_ini)는 J-Link halt 에서 N/A → 제거. main() 의 .get(기본값)이 관용 처리.
    'P9': {
        'interface':         'swd',
        'jlink_device':       'Cortex-R5',
        'sampler_type':       'jlink_halt',  # pylink 직접 halt (OpenOCD telnet desync 회피)
        'go_settle_ms':       50,      # NVMe 굶김 방지 (v5.6 "50ms 안정"). --go-settle 로 조정
        'jlink_speed':        4000,    # SWD 속도 (kHz)
        'jlink_ap_index':     0,       # APB-AP[0] (J-Link 확정: AP[0]=APB-AP 단일)
        'pc_reg_index':       None,    # connect 시 register_list 에서 R15/PC 자동 탐지
        ...
        'enable_ufas':        False,   # P9: UFAS 덤프 불가 (ufas_ini 키는 N/A → 생략)
...
MAX_SEQUENCE_CORPUS    = 50     # corpus 내 SequenceSeed 최대 개수 (초과 시 new_pcs 낮은 것부터 제거)
...
PCIE_PM_FUZZ_TARGETS = [
    ...
    ('exp', 0x10,  7, 1, 'extended_sync',       None),
    ('exp', 0x10,  8, 1, 'enable_clock_pm',     None),
    ('exp', 0x10,  9, 1, 'hw_autonomous_width', None),

    # PCIe Express Cap — DEVCTL2 (offset 0x28), 3 비트
    # Completion Timeout / CTO Disable 은 host PCIe controller / kernel timeout
    # 로직에 직접 영향 → fuzz 가 잡은 timeout 의 attribution 어려움 → 제외.
    ('exp', 0x28,  8, 2, 'ido',                 None),
    ('exp', 0x28, 10, 1, 'ltr_enable',          None),
    ('exp', 0x28, 13, 2, 'obff_enable',         None),

    # PCI PM Cap — PMCSR (offset 0x4), 3 비트 (D1/D2 forced + 2 normal)
    # D1/D2 PowerState 진입: 미지원 controller 면 명령 처리 못 함 → 일회성으로만
    # 처리 (D0 자동 복귀). PS3/PS4 forced_idle 패턴 차용.
    ('pm',  0x04,  0, 2, 'pmcsr_d1_d2_forced',  'forced_one_shot'),
    ('pm',  0x04,  3, 1, 'no_soft_reset',       None),
    ('pm',  0x04,  8, 1, 'pme_en',              None),

    # L1SS Cap — Control1 (offset 0x08), 3 비트
    # Common Mode Restore Time 은 큰 값(5~7) 시 wake 매우 느려져 명령 timeout 가능
    # → 제외. l1ss_enable / threshold value+scale 만.
    ('l1ss', 0x08,  0, 4, 'l1ss_enable',         None),
    ('l1ss', 0x08,  8, 8, 'ltr_l1_2_threshold',  None),
    ('l1ss', 0x08, 29, 3, 'ltr_l1_2_scale',      None),
]
# 총 13개 (12 normal + 1 forced)

# CLKREQ# timing perturbation modes (S2) — 4개 모두 --pm 활성 시 사용
CLKREQ_FUZZ_MODES = [
    # E2: hot-plug 시퀀스 모방 — L1.2 진입 후 wake 지연
    ('missed_wake',    {'delay_ms_min': 50,  'delay_ms_max': 500}),
    # E4: BIOS 설정 변경 범위 — spec T_POWER_ON ±50%
    ('extended_wait',  {'pct_min': 50, 'pct_max': 150}),
    # E1: hardware glitch level — 매우 짧은 pulse
    ('short_pulse',    {'pulse_us_min': 10, 'pulse_us_max': 100}),
    # E3: cheap PCIe slot glitch — 10ms 내 N회 toggle
    ('rapid_toggle',   {'count_min': 3,  'count_max': 8}),
]
