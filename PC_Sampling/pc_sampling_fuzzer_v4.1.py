...
class NVMeFuzzer:
    ...
    def _build_nvme_cmd(self, data: bytes, seed: Seed, input_file: str) -> List[str]:
        """NVMe CLI 커맨드 라인 구성 — Seed의 CDW 값 사용"""
        cmd = seed.cmd
        nvme_cmd = [
        ...
        # Seed의 CDW2~CDW15 값을 커맨드에 반영
        cdw_fields = [
            (2, seed.cdw2), (3, seed.cdw3),
            (10, seed.cdw10), (11, seed.cdw11),
            (12, seed.cdw12), (13, seed.cdw13),
            (14, seed.cdw14), (15, seed.cdw15),
        ]
        for idx, val in cdw_fields:
            if val != 0:
                nvme_cmd.append(f'--cdw{idx}={val:#x}')
    ...
    def _send_nvme_command(self, data: bytes, seed: Seed) -> Optional[int]:
        ...
        try:
            ...
            nvme_cmd = self._build_nvme_cmd(data, seed, input_file)
    ...
    def _save_per_command_data(self):
        ...
        for cmd_name in self.cmd_edges:
            ...
            data = {
                ...
                "traces": [[hex(pc) for pc in trace] for trace in traces[-50:]],
            }
