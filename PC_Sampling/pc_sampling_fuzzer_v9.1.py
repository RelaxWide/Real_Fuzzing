...
class NVMeFuzzer:
    ...
    def _llm_data_directive(self) -> str:
        """v9.1 #4: 구현된 needs_data 명령에만 valid 구조 data_hex 요청 — CDW 변이가 못 닿는
        data 파싱 코드 도달용. 전 명령 아님(미구현/no-data 제외). 대상 없으면 빈 문자열."""
        try:
        ...
        return ("\nFor these implemented data-transfer commands, ALSO emit a valid \"data_hex\" whose "
                "BYTES match the command's data structure (not just CDWs) — reaches data-parsing code "
                "that CDW mutation cannot: " + ", ".join(tgt[:8]) + ". "
                "e.g. Copy: Source Range Entries, 32B each (fmt 0h) <SLBA:8 LE><NLB:2><rsv:22>. "
                "Emit data_hex ONLY for these commands; omit it for others.")
            ...
            for n, st in self.cmd_stats.items():
                ...
                if n in self._unimpl_cmds:
                    ...
                    if hist:
                        ...
                        dig.append((rf, f"  {n} : {self._sc_name(dom)} → implemented; "
                                        f"fix the fields/values to pass validation and go deeper"))
            dig.sort(reverse=True)
            if dig:
                lines.append("\nPer-command device feedback (fix the exact issue; do NOT abandon "
    ...
    def _llm_build_request(self, task: str):
        ...
        if task == 'corpus_eval':
            ...
            rows = []
            for s in sample:
                rows.append(f"  {{\"seed_id\": {id(s)}, \"command\": \"{s.cmd.name}\", "
                            f"\"cdw10\": {s.cdw10}, \"new_pcs\": {s.new_pcs}, "
                            f"\"exec_count\": {s.exec_count}}}")
            user = ("Current corpus sample (seed_id + summary):\n[" + ",\n".join(rows) + "]\n\n"
                    "Task: for each seed_id emit an \"evaluations\" entry with \"score\" (0..1 "
                    "fuzzing value) and \"keep\" (bool). JSON only.")
            return self._LLM_SYSTEM, user
        return None
