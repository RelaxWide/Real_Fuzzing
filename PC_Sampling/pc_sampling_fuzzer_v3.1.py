#!/usr/bin/env python3
"""
PC Sampling 기반 SSD 펌웨어 Coverage-Guided Fuzzer v3.1

J-Link V9 Halt-Sample-Resume 방식으로 커버리지를 수집하고,
NVMe CLI를 통해 SSD에 퍼징 입력을 전달합니다.

v3.1 변경사항:
- 커버리지 자동 저장 (coverage.txt) - 주기적 + 종료시
- JSON 통계 자동 저장 (stats.json)
- 버전 표시 수정 (v2 → v3.1)
"""
...
class NVMeFuzzer:
    ...
    def _save_stats(self, stats: dict):
        """통계를 JSON 파일로 저장 (v3.1 추가)"""
        stats_file = self.output_dir / 'stats.json'
        with open(stats_file, 'w') as f:
            json.dump(stats, f, indent=2)

    def _save_coverage(self):
        """커버리지를 파일로 저장 (v3.1 추가)"""
        coverage_file = self.output_dir / 'coverage.txt'
        saved = self.sampler.save_coverage(str(coverage_file))
        log.debug(f"[Coverage] Saved {saved} PCs to {coverage_file}")

    def _periodic_save(self):
        """주기적으로 커버리지와 통계 저장 (v3.1 추가)"""
        self._save_coverage()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='PC Sampling SSD Fuzzer v3.1')
    ...
    parser.add_argument('--save-interval', type=int, default=SAVE_INTERVAL,
                        help='Save coverage/stats every N executions')
