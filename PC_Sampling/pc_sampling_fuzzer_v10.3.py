#!/usr/bin/env python3
"""
PC Sampling 기반 SSD 펌웨어 Coverage-Guided Fuzzer v10.3
...
- LLM:      로컬 vLLM(OpenAI 호환) 기반 spec-aware 시드/시퀀스 생성·corpus 평가
            (--rag, 백그라운드). 백엔드는 rag.module_path 로 교체 가능.
...
- v10.3: LLM 백엔드를 사내 2노드 Samba 브리지 → **로컬 vLLM** 으로 이관. task별 json_schema
         구조화 출력, {raw,diagnostics} 반환 계약, 실패 집계 분리. 계획=docs/V10_3_LLM_BACKEND_PLAN.md
- v10.2: 실행 근거 기반 LLM 목표 탐색, setup 보존, JSON 생성 규칙, 성과 기반 task 배분.
- v10.1: BM9K1 전용 Extended SMART(Get Log Page LID C0h) state 관측 + 벤더 로그 **전체
         레이아웃 출력**(log_layouts). 기존 state 필드 중 자기유발/무정보 4개 제외(BM9K1 세트만).
- v10.0: BM9K1(RISC-V) 커버리지 — 코드 오버레이 bank 구분, 코어별 리포트, LLM 프롬프트 개선.
...
# v10.1: 벤더 로그 페이지 **전체 레이아웃** 표 (선택). 모니터링 필드는 그 로그의 일부만
#   보지만, 스냅샷 출력에서는 표가 있는 LID 에 한해 **전 필드**를 찍는다. 표가 없는 LID
#   (0x01/0xDF 등)는 종전대로 모니터링 offset 만 출력한다(레이아웃 미상 → 추측 금지).
#   {lid:int → {'name':str, 'fields':[{'offset','length','name'}, ...]}}
LOG_LAYOUTS = {}
for _ly in (_CFG.get('log_layouts', {}) or {}).get('vendor_logs', []) or []:
    try:
        LOG_LAYOUTS[int(_ly['lid'])] = {
...
class Seed:
    ...
    learning_meta: dict = field(default_factory=dict)  # v10.2 request/target/generator context
...
_LLM_TOP_KEYS = ('seeds', 'sequences', 'evaluations', 'io_workload', 'generators')
...
class _V101Fuzzer:
    ...
    def _calibrate_seed(self, seed: Seed) -> Seed:
        ...
        _learning_cal_seen = set()
        _learning_cal_global = set(self.cov.covered_bbs if _cal_per_core else
                                   self._sa_covered_bbs if _cal_static_bb else
                                   self.sampler.global_coverage)
        _learning_cal_valid_runs = _learning_cal_gain = 0
        for run_i in range(total_runs):
            ...
            _learning_keys = (_projected.seed_keys if _cal_per_core else
                              _run_bbs if _cal_static_bb else set(self.sampler.current_trace))
            _learning_new = len(_learning_keys - _learning_cal_global - _learning_cal_seen)
            _learning_cal_seen.update(_learning_keys)
            _learning_valid = (bool(_learning_keys) and self._learning_window_valid
                               and self._last_nvme_status is not None and rc != self.RC_SKIP)
            if _learning_valid:
                _learning_cal_valid_runs += 1
                _learning_cal_gain += _learning_new
            self._learning_observe(seed, self._last_nvme_status, rc, _learning_keys,
                                   _learning_new, 'calibration', False)
        ...
        if _new_count:
            ...
            if self.learning.enabled and _runtime_cal and _learning_cal_gain:
                _origin = _cal_src.partition('/')[0]
                self._boost_gain[_origin] = self._boost_gain.get(_origin, 0) + _learning_cal_gain
    ...
    def _stop_sampling_checked(self, context: str = "command") -> tuple[int, bool]:
        ...
        self._learning_window_failed = bool(err and err.is_set())
    ...
    def _account_command(self,
        ...
        if rc == self.RC_SKIP:
            self._learning_observe(seed, None, rc, set(), 0, source, seq_member)
        ...
        try:
            ...
            if _bf in ('cmd', 'seq') and not self.learning.enabled:
        ...
        if self.learning.enabled and not self._learning_window_valid:
            # Keep global observations, but do not reward an interrupted sampling window.
            is_interesting, new_pcs = False, 0
        if (self.learning.enabled and self._learning_window_valid and _seed_covered
                and _led_sc is not None):
            _bo, _, _bf = self._cov_src_tag(seed, source, seq_member=seq_member).partition('/')
            if _bf in ('cmd', 'seq'):
                self._boost_exec[_bo] = self._boost_exec.get(_bo, 0) + 1
        ...
        self._learning_observe(seed, _led_sc, rc, _seed_covered, new_pcs, source, seq_member)
        ...
        if rc == self.RC_ERROR:
            ...
            if self._seq_sink is not None and seq_member:
                _err_seed = Seed(
                    ...
                    force_admin=seed.force_admin,
                    data_len_override=seed.data_len_override,
                    prov_id=_led_prov, learning_meta=self._learning_meta_for(seed),
    ...
    def _finalize_seq_sink(self) -> None:
        ...
        if self._seq_sink['interesting']:
            ...
            _seq_seed = SequenceSeed(
                ...
                prov_id=getattr(_bs, 'prov_id', None),
                learning_meta=dict(getattr(_bs, 'learning_meta', {}) or {}),
            )
        ...
        try:
            while True:
                ...
                if (self.config.io_workload_enabled and IO_WL_ENABLED
                        and not self._learning_setup_active()
                ...
                if not is_det_stage:
                    ...
                    if self._pending_seq_seeds:
                        ...
                        mutated_seed = self._learning_seq_member(_next_seed)
                        ...
                        if self.corpus and random.random() >= self.config.random_gen_ratio:
                            ...
                            if base_seed is None:
                                ...
                                if self._seq_cmds_in_window < SEQ_MAX_PER_100:
                                    ...
                                    _preserve_setup = self._learning_seq_start(base_seed)
                                    _first = self._learning_seq_member(_cs_cmds.pop(0), first=True)
                                    _ctx_mode = self._CTX_SEQUENCES.get(_cs_names)
                                    if _ctx_mode is not None and not _preserve_setup:
                                        # 첫 명령 mutation 결과에서 ctx 파생
                                        self._pending_seq_ctx = {
...
class NVMeFuzzer(LearningMixin, _V101Fuzzer):
    """v10.2: preserve the v10.1 device paths, extend its LLM execution feedback."""
    _learning_config = _RAG.get('learning', {})
    _learning_max_bytes = MAX_INPUT_LEN
    _learning_max_seeds = RAG_MAX_SEEDS
