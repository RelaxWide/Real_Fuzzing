...
>
> ⚠ **이 문서는 조사 이력이다. 현황·현재 판단은 [`STATUS.md`](STATUS.md) 가 최신이고,
> 충돌하면 STATUS 가 우선한다.** 아래에는 **halt/PC 샘플링을 전제로 쓴 낡은 서술**이
> 남아 있다 — 그 전제는 폐기됐다(T32 도 halt 를 쓰지 않음이 `cpuaccess DENIED` 로 확인).

**목표:** 신제품(RISC-V 기반)에 퍼저를 올린다. NVMe·코퍼스·LLM·replay 등은 주소 무관 →
그대로 재사용하고, **커버리지 수집 계층만 새로 만드는 것**이 v10.0 의 범위다.
~~샘플러 층만 교체~~ 는 halt/PC 샘플링 전제의 낡은 서술이라 **철회한다** — 실제로는
**트레이스(온칩 sink) 기반**이라 수집 계층 자체가 다르다(`STATUS.md` §7 Phase 0).
...
~~halt 샘플러가 돌기 시작한 뒤~~ → **트레이스가 주 경로**이므로 이것은 나중이 아니라
**DM/SBA 가 열리는 즉시 쓸 인터페이스**다.
...
호스트 프리즈)가 한 번에 해소된다.
**갱신:** T32 커버리지가 halt 방식이 아님이 확인돼, 트레이스는 이제 **병행 트랙이 아니라
주 트랙 후보 중 하나**다(`STATUS.md`). 다만 외부 streaming trace 면 J-Trace PRO 가 필요하다.
...
> **주 트랙은 `STATUS.md` 의 C1 — T32 커버리지 메커니즘 식별이다.**
> 아래는 halt 대안 트랙 진행 상황이다.

`verify_halt_pc.py` 로 halt → PC → resume 검증 — **현재 halt 실패로 막힘**(APB=0 범위).
...
> ⚠️ **이 절은 "halt PC 샘플링을 주 경로로 가정" 하던 시기의 판단이다.**
> **T32 커버리지가 halt 방식이 아님이 확인돼 전제가 바뀌었다.**
> 현재 주 트랙과 게이트는 `STATUS.md` 를 볼 것. 아래는 **보조 트랙(대안)** 의 로드맵으로만
> 유효하다. 기록으로 남긴다.
...
**(보조 트랙 기준) 0단계만 되면 halt 샘플러로는 퍼저가 돈다.**
다만 T32 가 쓰는 방식이 확정되기 전에는 이것이 v10.0 의 단일 착수 조건이 아니다. 트레이스는 성공하면 halt 를 대체하고, 실패해도 v10.0 은 이미 동작.
