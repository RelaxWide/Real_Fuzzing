...
## 3.5 ★★★ 최종 실측 — 인터커넥트가 **미매핑 응답**을 돌려준다

전부 **읽기 전용**이고 전송은 정상 완료된다(오류 비트 없음, `TrInProg=0`).

**AP 레지스터 (전부 정상)**
```
APBAP1/APBAP2   IDR=0x09130006  DeviceEn=1  Prot=0x00  BASE=0x00000003
AXIAP1          IDR=0x09130004  DeviceEn=1  Prot=0x30  BASE=0x00000002
DPIDR(=TAP IDCODE) = 0x6BA0009D    DP CTRL/STAT = 0xF0000000 (양쪽 전원 ACK)
주소 디코드 폭: APBAP1/2 는 **wrap 없음(32비트 전부)**
```

**★ 결정적 관측 — 응답이 주소가 아니라 16바이트 정렬만 따라간다**

APBAP1 으로 여러 주소를 32비트 읽은 결과 **14/14 가 예외 없이** 이 규칙을 따릅니다:

| 주소 | 값 |
|---|---|
| `0x81480000`, `0x81480010`, `0x81480100`, `0x81480400`, `0x81480800`, `0x81480C00`, `0x81480FF0` | 전부 `0x00000001` |
| `0x81480044`, `0x81480FFC`, `0x0021BFFC`, `0xFD18001C` | 전부 `0xEAFFFFFE` |

즉 버스가 **128비트 한 줄을 고정값으로** 돌려주고 저희 32비트 읽기가
그 lane 을 집어오는 형태입니다:
```
00000001  EAFFFFFE  EAFFFFFE  EAFFFFFE
```
**이는 인터커넥트의 default slave 응답으로 보입니다** — 매핑되지 않은 주소에
고정 라인을 돌려주는 것. 메모리도 레지스터도 아닙니다.

**★★ 그런데 ROM 은 그 자리를 가리킵니다**
```
APBAP1 BASE = 0x00000003        → ROM 테이블 @ AP 주소 0x0
AP[0x0] = 0x81480003            → 컴포넌트 @ 0x81480000   (APBAP2 는 0x81481003)
AP[0x4] = 0x00000000            → 테이블 끝
```
`0x0`/`0x4` 는 위 default 라인이 **아닙니다**(lane 규칙과 어긋납니다).
즉 **APBAP1 에서 실제로 매핑된 것은 앞 8바이트(ROM)뿐이고,
ROM 이 가리키는 `0x81480000` 을 포함해 나머지는 전부 미매핑**입니다.

**AXI-AP** 로 `0xC81040`/`0xC81044`(TRACE32 가 리셋 해제에 쓰는 주소)를 읽으면
`0`만 나오고 `CSW.Prot` 13종을 바꿔도 동일합니다.

### 저희 해석과 막힌 지점

**ROM 은 컴포넌트를 등록하고 있는데 인터커넥트는 그 주소를 매핑하지 않습니다.**
컴포넌트가 **전원/리셋/아이솔레이션 상태**에 있으면 이 그림이 됩니다.

TRACE32 는 `Data.Set AXI:0xC81040 %LE %Long 0x13333` 로 리셋을 해제합니다.
**그런데 그 AXI 주소도 저희에게는 `0` 으로만 읽힙니다** — 닭과 달걀입니다.
...
>    나옵니다(`Prot` 13종 무관).
>    **TRACE32 는 그 AXI 접근 전에 무엇을 합니까?**
>    (전원/클럭 인에이블, 다른 AP 경유, 별도 unlock, 특정 순서 등)
> 5e. APB-AP 의 `BASE=0x3` 이 `AP 주소 0x0` 의 ROM 을 가리키고, 그 ROM 엔트리
>    `0x81480003` 이 `0x81480000` 을 가리키는데 **그 영역은 64워드 연속으로
>    균일**합니다. **그 컴포넌트를 살리려면 무엇이 선행되어야 합니까?**
