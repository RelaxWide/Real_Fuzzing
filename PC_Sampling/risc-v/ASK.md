...
## 3.5 ★★ 최종 실측 — 자기모순에 도달했다

전부 **읽기 전용**이고 **전송은 정상 완료**된다(66/66, DP 오류비트 없음,
`TrInProg=False`). 즉 아래 값들은 **진짜 읽은 데이터**다.

**AP 레지스터 (전부 정상)**
```
APBAP1/APBAP2   IDR=0x09130006  DeviceEn=1  Prot=0x00  CFG=0x00010000  BASE=0x00000003
AXIAP1          IDR=0x09130004  DeviceEn=1  Prot=0x30  CFG=0x00010000  BASE=0x00000002
```

**APB-AP: ROM 은 있는데 그것이 가리키는 곳이 비어 있다**
```
BASE = 0x00000003        → present=1, 32비트 포맷, ROM 테이블 @ AP 주소 0x0
AP[0x0]  = 0x81480003    → present=1, 컴포넌트 @ 0x81480000   (APBAP2 는 0x81481003)
AP[0x4]  = 0x00000000    → 테이블 끝 (엔트리 하나)
0x81480000 부터 64워드 연속 덤프 → **UNIFORM (전부 같은 값)**
0x81480044 (dmstatus 자리) = 0xEAFFFFFE
0x81480FF0~FFC (CIDR 자리) = 0xEAFFFFFE
```
J-Link 도 같은 것을 본다: `Unsupported or invalid DM version 14 detected`
(=`0xE`, 즉 `0xEAFFFFFE` 의 하위 니블).

**AXI-AP: 전부 0. `Prot` 13종을 바꿔도 동일**
```
0xC81040 / 0xC81044 (TRACE32 가 리셋 해제에 쓰는 주소) = 0x00000000
BASE = 0x00000002 → ROM 없음
```

### 우리가 세운 해석과 막힌 지점

ROM 테이블은 **항상-켜짐 디버그 도메인**에 있고, 그것이 가리키는 컴포넌트는
**리셋/언클럭 도메인**에 있는 것으로 보입니다. 리셋에 잡힌 컴포넌트는 ROM
목록에는 남고 읽으면 균일한 값이 나오기 때문입니다.

TRACE32 는 그것을 `Data.Set AXI:0xC81040 %LE %Long 0x13333` 로 풉니다.
**그런데 그 AXI 주소도 저희에게는 `0` 으로만 읽힙니다.**

> **닭과 달걀입니다.** 리셋을 풀 레지스터가 AXI 에 있는데 AXI 가 (아마 같은
> 이유로) 안 보입니다. **TRACE32 에서는 같은 AXI 접근이 동작합니다.**
> 그 차이가 무엇인지가 저희가 답을 못 찾은 지점입니다.
