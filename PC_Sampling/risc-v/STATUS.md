...
### P0 — `probe_axi.py` : AXI-AP 로 리셋 블록을 **읽는다**

```bash
sudo python3 probe_axi.py
```

`0xC81040` / `0xC81044` 와 그 주변을 **AXIAP1** 로 읽는다.
도구가 구조적으로 **`DRW` 에 쓰지 않는다**(CSW/TAR 만 건드린다 — 검증함).

| VERDICT | 의미 |
|---|---|
| **`AXI_READABLE`** | ★★★ **"AP 너머는 전부 죽어 있다" 는 결론이 틀렸다.** APB 만 죽어 있었다. 리셋 컨트롤러 값으로 코어 상태를 판정할 수 있다 |
| `AXI_ALL_DEAD` | APB 와 같은 상태 → 리셋 가설을 다시 봐야 한다 |

**읽히는 것만으로 큰 진전이다** — AXI-AP 경로가 살아 있다는 증명이고,
그러면 트레이스 블록도 AXI 로 다시 시도할 근거가 생긴다.

### P0.1 — 읽은 뒤에 판단할 것

`0xC81040` 값이 `0x13333` 과 다르면 **일부 코어가 리셋 상태**일 수 있다.
그때 쓰기 여부는 **별도 판단**이다 — 동작 중인 SSD 를 리셋할 위험을
감수할지, 아니면 그 정보만 들고 벤더에 확인할지.

> `dmver.sh --aps 3`(AHB) 은 version 보고가 아예 없었다(`-`).
> AHB 셀렉터로는 그 단계까지 못 간다는 뜻이다.
