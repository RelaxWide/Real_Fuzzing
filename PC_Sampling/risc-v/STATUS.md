...
>
> ★ **§0.5 가 현재 결론이다.** 아래 절들은 조사 이력이고, 무효 세션이나
> 잘못된 게이트 위에서 쓴 낡은 서술이 섞여 있다. 충돌하면 §0.5 가 우선한다.
...
- T32 에서 `clavis` **직전** `0x81480000` 읽기 → 무응답이어야
- `clavis` **성공 직후** 같은 주소·같은 접근클래스 읽기 → **정상 `dmstatus` 로
  바뀌면 = 인증이 DM 을 여는 직접 증명** → **투자 정당화 완료**
- 안 바뀌면(다른 T32 단계가 DM 을 살림) → 인증 구현은 헛수고, 게이트 재탐색
