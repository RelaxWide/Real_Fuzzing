...
**① 66번 전송이 전부 정상 완료됐다. 오류비트도 없다.**
⇒ `0xEAFFFFFE` / `0x00000001` / `0x00000000` 은 **진짜 읽은 데이터**다.
내가 "버스 기본값 = 읽은 게 아님" 으로 분류한 것이 **틀렸다.**
**버스는 응답하고 있고, 그 주소에 그 값이 실제로 있다.**
(`MEM_AP_TRANSFER_BROKEN` 결론도 철회한다)
...
### P0 — **여기서 자력 탐색을 끝낸다.** `ASK.md` 를 보낸다
