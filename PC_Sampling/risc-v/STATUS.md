...
### P0 — 만들어 놓고 **한 번도 안 돌린** 도구가 있다

```bash
sudo python3 probe_rom_dm.py --brief
```

`probe_rom_dm.py` 는 정확히 이 질문용으로 만들었는데 실행한 적이 없다.
지금 우리가 모르는 것은 **전송이 어느 단계에서 어떻게 끝나는가**다:

```
CSW.TrInProg          전송이 아직 진행 중인가
DRW 읽기 후 DP 오류    STICKYERR / WDATAERR / STICKYORUN
ABORT 로 해제되는가
CSW 원본 / Prot / CFG / BASE
```

**"기본값이 돌아온다" 와 "버스가 에러를 낸다" 는 완전히 다른 상태다.**
전자면 그 주소에 아무것도 없는 것이고, 후자면 **접근이 거부되는 것**(권한·보안·
전원)이다. **그 구분을 한 번도 안 했고, 그게 다음 수를 정한다.**

이번에 `AXIAP1` 과 리셋 컨트롤러 주소(`0xC81040`/`0xC81044`)도 대상에 넣었고,
TAP 수정 이후 버전(API_LEVEL 5)이라 **정상 TAP 위에서 돈다.**

출력의 이 두 줄이 핵심이다:
```
전송단계: {'ok': n, 'drw_fail': n, 'tar_mismatch': n, ...}
DP오류비트: {'STICKYERR': n, ...}
```

| 결과 | 의미 | 다음 |
|---|---|---|
| `STICKYERR` 가 뜬다 | **버스가 거부한다** — 권한/보안/전원 문제 | `Prot`/`SDeviceEn` 조합, 벤더 확인 |
| 오류비트 없음 + 기본값 | 버스는 응답하는데 **그 주소에 아무것도 없다** | 주소 체계 자체가 다르다 |
| `TrInProg=True` | 전송이 안 끝난다 | 클럭/전원 |
