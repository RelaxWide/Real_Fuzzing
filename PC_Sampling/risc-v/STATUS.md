...
```bash
sudo python3 probe_axi.py --prot-sweep
```

AXI 는 **AxPROT/AxCACHE 로 접근 권한이 정해지고, 권한이 안 맞으면
에러 없이 `0` 을 돌려준다(RAZ).** 지금 AXI 가 **전부 0 이고 오류비트도 없는 것**이
정확히 그 모양이다. `Prot` 후보 13종으로 같은 주소를 읽는다.

| VERDICT | 의미 |
|---|---|
| **`PROT_FOUND`** | ★★★ 접근 권한이 원인이었다. 그 `Prot` 으로 전 도구를 맞춘다 |
| `PROT_NO_EFFECT` | 권한 문제가 아니다 → 그 주소가 이 AP 의 주소공간에 없다 |

**여전히 쓰기는 하지 않는다** — `DRW` 에 쓰지 않는 것을 정적으로 확인했다.

### 남은 수수께끼 하나

APB ROM 엔트리가 `0x81480000` 을 가리키는데 거기를 읽으면 `0x00000001` 이다.
CoreSight 컴포넌트라면 `0xFF0~0xFFC` 에 CIDR 이 있어야 하는데 `0xEAFFFFFE` 다.
**ROM 은 진짜인데 그것이 가리키는 곳이 이상하다** — ROM 엔트리의 주소 필드는
**ROM 베이스로부터의 부호 있는 오프셋**이라, 해석을 다시 볼 여지가 있다.
