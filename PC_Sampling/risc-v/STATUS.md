...
```bash
sudo python3 probe_axi.py --ap-name APBAP1 --window --win-base 0x81480000 --count 64
```

64워드(256바이트)를 연속으로 읽고 **값이 바뀌는 지점만** 출력한다
(전사량을 줄이려고 run-length 로 압축한다).

```
0x81480000~0x8148000C = 00000001
0x81480010~0x8148002C = EAFFFFFE
0x81480030            = 00001234      ← 이런 게 보이면 (b)
```

| VERDICT | 의미 |
|---|---|
| **`STRUCTURED`** (고유값 3종 이상) | ★★★ **레지스터가 있다.** "아무것도 없다" 가 틀렸고, 구간 경계가 레지스터 경계를 알려준다 |
| `UNIFORM` | 이 영역은 정말 비어 있다 → 다른 base 나 다른 AP |

`--win-base 0x81481000`(Ncore), `--ap-name APBAP2` 로도 같이 본다.
