...
### ❌ AXI 다섯 주소 **전부 0, 오류 없음** (2026-08-11)

```
0xC81024 FCore  en bit0        = 00000000
0xC81028 NCore0 b0/NCore1 b16  = 00000000
0xC8102C CMCore en bit0        = 00000000
0xC81040 MPCore clk/rst        = 00000000
0xC81044 NCore  clk/rst        = 00000000
STICKYERR / WDATAERR / STICKYORUN 없음
```

**feedback 의 판정표가 이 경우를 미리 정해뒀다:**
> *"정상 disabled 값인지 RAZ/security 인지 **구분 불가**.
>  CMM 으로 더 못 좁힘. SoC 레지스터 사양 또는 실제 T32 측정값 필요"*

### ★ 왜 구분이 안 되는가 — **positive control 이 없다**

- T32 는 **HCore 를 AXI 를 읽지 않고 무조건 enabled** 로 처리한다
  → HCore 는 positive control 로 **쓸 수 없다**
- SSD 가 동작한다는 사실은 **"HCore 는 살아 있다"** 까지만 말해준다
- **F/CM/NCore 가 실제로 disabled 라면 `0` 이 정상값**이다

⇒ **"0 을 읽었다" 만으로는 AXI 경로의 성패를 가를 수 없다.**
이건 측정을 더 해서 좁힐 수 있는 종류가 아니다 — **기댓값을 모르기 때문**이다.

### ⇒ 자력 탐색의 종료 조건에 도달했다

이번 판단은 **내 감이 아니라 feedback 이 미리 정해둔 기준**이다.
그 기준의 마지막 칸에 정확히 들어왔다.

**필요한 것은 둘 중 하나뿐이다:**

| | |
|---|---|
| **①** | 이 다섯 레지스터의 **설계상 정상값** (SoC 레지스터 사양) |
| **②** | **T32 로 같은 주소를 읽은 실측값** (같은 보드에서) |

**②가 더 쉽고 결정적이다.** T32 를 한 번만 붙일 수 있으면
`Data.Long(AXI:0x00C81024)` 다섯 줄로 끝난다. 그 값과 우리 값이 같으면
**AXI 경로는 정상**이고 문제는 APB/DM 쪽으로 확정된다. 다르면 **AXI 경로가
문제**임이 확정된다. **어느 쪽이든 지금 갈린 채로 남은 것이 갈린다.**
...
> **2026-08-11 현재:** 자력 측정의 종료 조건에 도달했다(§7.5).
> 아래는 그때까지 확정된 사실이다.
