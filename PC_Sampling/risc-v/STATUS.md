...
### P0 — DM 컴포넌트를 **직접 읽는다** (ROM 이 가리키는 곳)

```bash
sudo python3 probe_ap_raw.py --addrs dm --no-rom --sessions 3
```

ROM 엔트리가 `0x81480000` 을 가리켰다. **거기에 정말 있는지** 확인한다.
`APBAP1` 로 그 컴포넌트의 CoreSight ID 와 DMI 후보를 읽는다:

```
+0xFF0 CIDR0  기대 0x0D        +0xFF8 CIDR2  기대 0x05
+0xFF4 CIDR1  하위니블 0x0     +0xFFC CIDR3  기대 0xB1
       CIDR1 상위니블 = class (9 = CoreSight 컴포넌트, 1 = ROM 테이블)
+0xFE0/0xFE4 PIDR0/1          ← 제조사·파트번호
+0x040 / +0x044               dmcontrol / dmstatus  (stride <<2 가정)
+0x010 / +0x011               같은 것  (stride 1 가정)
```

| 결과 | 결론 |
|---|---|
| **CIDR = `0D / _0 / 05 / B1`** | ★★ **CoreSight 컴포넌트 확정.** 거기가 DM 이다 → stride 를 실측으로 가른 뒤 J-Link 설정 확정 |
| `dmstatus` 후보의 `version`=2·3 | ★★★ **DM 도달.** 블로커 해소 |
| 전부 고정 상수 | ROM 이 가리키는 곳이 이 AP 로는 안 보인다 → 접근 폭/윈도 문제 |

> ⚠ `0xEAFFFFFE` / `0x00000001` / `0x00040700` 이 나오면 **읽은 게 아니다**(§실측).
...
| **C3c `CSW.DeviceEn` 판정** | ✅ **=1, 잠금 아님** (2026-08-11) | | |
| **C3d `mem_read32` 양성 대조** | ⬜ ← **다음 측정** (펌웨어 코드 영역) | | |
| C4 raw 트레이스 1회 회수 | ⬜ | | |
| C5 T32 결과와 교차 검증 | ⬜ | | |
| C6 퍼저 반복·복구·성능 | ⬜ | | |
...
**디버그 하드웨어는 전부 정상이다** — DP·DAP 전원·AP 6개(IDR 6/6 일치)·AP 레지스터
R/W·**`CSW.DeviceEn=1`(잠금 아님)**. 그런데 **메모리 트랜잭션만 전부 실패**한다.
남은 갈림길은 **"우리 `mem_read32` 가 원래 안 되는 것"** vs **"그 주소들이
디코드되지 않는 것"** 이고, **펌웨어 코드 영역(`0x0~0x21BFFF`)을 읽는 양성
대조**가 이를 가른다. 읽히면 트레이스 블록 결과를 신뢰할 수 있고, 그게 읽히면
**DM 을 우회해 커버리지 파이프라인을 바로 세운다.**
