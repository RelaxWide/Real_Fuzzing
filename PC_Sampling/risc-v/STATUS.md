...
### P0 — `probe_alias_power.py` : 물어보지 않고 풀 수 있는 두 가설
...
#### [A] AP 주소 디코드 폭 (aliasing)

```
AP 0x0 을 읽으면        → 0x81480003   (DM 을 가리키는 값)
0x81480000 을 읽으면    → 무응답 상수
```
AP 창이 하위 N비트만 디코드하면 `0x81480000` 이 잘려 엉뚱한 곳으로 간다.
**그런데 TAR 되읽기는 통과한다** — TAR 은 버스가 무시하는 비트도 담기 때문이다.
지금 증상과 정확히 맞는다.

★ **주소를 추측하지 않고 wrap 지점을 찾는다.** N비트를 디코드하면 주소 `2^N`
은 주소 `0` 과 같은 곳을 가리킨다. `0x0` 은 이미 live 값을 주니 이걸 기준으로:

```
ref = read(0x0);  for N in 12..31: read(1<<N) == ref ?
우연 배제: read((1<<N)+4) == read(4) 도 **동시에** 맞아야 wrap 으로 인정
가장 작은 N = 디코드 폭 W  →  DM 의 진짜 주소 = 0x81480000 & (2^W - 1)
```
그 자리를 바로 읽는다. **맞으면 벤더 없이 끝난다.**

#### [B] 디버그 전원 사이클 (T32 의 `DOWN → 500ms → UP`)

T32 HCore 시퀀스에 있는데 **우리는 전원을 내렸다 올린 적이 한 번도 없다.**
`CDBGPWRUPREQ` 를 내리고 500ms 기다렸다 다시 올린다(T32 처럼 **DBG 만**).
리셋 없이 DM 을 깨우는 시퀀스일 수 있다.

| VERDICT | 의미 |
|---|---|
| `ALIAS_W<n>_..._DMLIVE` | ★★ **끝.** 디코드 폭이 원인이었고 DM 주소가 나왔다 |
| `ALIAS_W<n>_..._DMDEAD` | 폭은 좁은데 그 자리도 죽음 → 다른 창 |
| `NO_ALIAS_...` | 32비트 다 디코드 → aliasing 반증. [B]/질문으로 |
| `PWRCYCLE_OPENED_...` | ★★ **전원 사이클이 열었다.** 그 시퀀스를 연결 절차에 넣는다 |
| `NO_SIGNAL` | 둘 다 아님 → 남은 건 ⑦⑧(벤더 질문)뿐 |

> 합성 메모리(폭 24/28/wrap 없음)로 탐지 로직을 검증했다.
