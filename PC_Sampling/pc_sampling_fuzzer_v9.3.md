...
- 4번째 task `io_patterns`(기존 3 task 와 함께 **가중 라운드로빈 상시 포함** — 설계 의도: additive, 기존 탐색 대체 아님). 비중은 `rag.task_weights.io_patterns`(기본 2)로 조절(design §9). `_pending_workload` 있으면 소비 전까지 재요청 안 함(중복 적체 방지).
