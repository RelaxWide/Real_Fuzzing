#!/usr/bin/env python3
"""
PC Sampling 기반 SSD 펌웨어 Coverage-Guided Fuzzer v7.0
        ...
        for i, combo in enumerate(_preflight_order):
            ...
            try:
                if combo.pcie_d == PCIeDState.D3:
                    ...
                    # L1.2 복귀 순서: CLKREQ# assert → L1(클락 on) → PS0 → L0
                    # CLKREQ# assert 후 링크는 L1으로 전환(클락 on, NVMe 접근 가능).
                    # L0 레지스터 쓰기(링크 재협상) 전에 PS0 먼저 전송해야 안정적.
        ...
        try:
            while True:
                ...
                if (self.config.pm_inject_prob > 0
                    ...
                    # enlat 기반 최소 진입 시간으로 세팅 + 명시적 idle → 실제 NOPS 진입 보장
                    if random.randint(0, 5) == 0:
                        ...
                        _next_combo = None  # 일반 combo 로테이션 스킵
                    else:
                ...
                if (self.config.state_enabled
                    ...
                    if _snap is not None:
                        if self._state_snap_prev is not None:
                            _delta = self.state_monitor.delta(
                                self._state_snap_prev, _snap)
                            if (_delta.is_interesting
                                    and self.state_monitor.update_cov_map(
                                        _delta, self.state_cov_map)
