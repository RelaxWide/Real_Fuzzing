...
찾으므로, **PDF 별 하위 폴더를 둔 채 상위 폴더 하나만** 넘기면 된다 — 쪽수로 쪼갠 PDF 를
다루는 가장 편한 방법이다.
...
**소스 식별자는 `상위폴더/파일명`** 이다. basename 만 쓰면 PDF 별 폴더에 같은 이름의
분할 파일(`part1.jsonl` …)이 있을 때 서로 다른 파일이 하나로 합쳐져, manifest 가 무엇이
들어갔는지 못 말하고 "이 소스는 안 바뀌었다" 판정도 엉뚱한 해시로 하게 된다. 절대 경로를
안 쓰는 이유는 재사용 안정성이다 — 트리를 옮기거나 다른 디렉터리에서 실행하면 식별자가
통째로 바뀌어 전량 재임베딩이 된다. 두 단계로도 겹치면 **게시 전에 거부**한다.

**게시 전 검증** — 소스 식별자 중복, 본문↔벡터 개수, 차원, NaN/Inf, 영벡터, `doc_id` 중복. 하나라도
