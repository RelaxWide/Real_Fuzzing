...
원본 guide에서는 `response.json()`과 첫 검색 결과 반환을 찾아 검증 코드를 삽입한다.
두 문장이 붙어 있지 않아도 되며, 중간 로그, hits/문서 변수 분리, try/finally 구조를
지원한다. 원래 HTTP 호출, 로그와 결과 추출 문장은 보존한다. 여러 JSON 응답이나
불명확한 분기는 추측해서 수정하지 않는다.
...
71개 회귀 테스트 통과. 실제 퍼저 요청 래퍼, 마커 오류와 구버전 입력, HTTP 200 토큰 오류,
...
### 설치 도구의 `Unexpected retrieval layout` 오류

이전 도구는 검색 함수의 마지막 두 문장이 JSON 할당과 직접 return이어야 했다.
최신 도구는 해당 위치 제약을 제거했다. 설치 전에 실제 guide 절대 경로, 함수 이름,
줄 번호, 문장 종류를 출력하므로 다른 파일을 선택했는지도 확인할 수 있다.
이 오류가 난 실행에서는 guide가 수정되지 않는다. 최신 설치 도구로 같은 명령을 다시 실행한다.
여전히 지원하지 않는 구조라면 원인 안내와 함께 변경 없이 종료한다.
