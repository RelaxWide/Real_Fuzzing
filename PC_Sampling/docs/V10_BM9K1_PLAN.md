...
2. **셔플** (예 `[2,0,1,0,3,0]`)
3. 버스트 **경계**에 지터(±20%) — 주기적 펌웨어 루프와의 aliasing 방지.
   ⚠ 지터는 **버스트 사이(셋업 지점)에만**. 버스트 **안쪽 DRW 루프에는 지연을 넣지 않는다**
   — 아래 §실기 검증된 폴링 제약 참조.

가중치(=관측량)와 순서(=편향)를 분리한다. `primary_core`(NVMe 핸들러)는 **실측으로 확정**한다 —
이름만으로는 알 수 없다.

### ★ 실기 검증된 폴링 제약 (2026-09) — 설계 구속조건

두 가지가 실측으로 확정됐고, 둘 다 **추측이 아니라 반증된 결과**다.

**① I/O 부하 중 붕괴 → "셋업 1회 + DRW만 반복" 이 유일한 동작 형태**
매 샘플 풀 셋업을 하던 방식은 I/O 부하가 걸리면 제대로 동작하지 않았다. FIFO 모드로
**셋업을 1회만** 하고 이후 `DRW` 만 반복 읽는 형태로 바꿔야 하며, **그 루프에 아무것도
끼워넣지 않는 것**이 조건이다. 넣었다가 실패율이 폭증해 **전부 제거한** 것들:

| 넣었던 것 | 결과 |
|---|---|
| 주기적 `sbcs`(SBA 에러) 확인 | ❌ 실패율 폭증 |
| `clear_sticky()` 복구 | ❌ 실패율 폭증 |
| interval 지연(sleep) | ❌ 실패율 폭증 |

→ 폴링 루프 본체는 `dap.ap_read(DRW)` **한 줄**이어야 한다.
> 이는 앞서 코드리뷰에서 권했던 "루프 끝에 SBCS 를 읽어 에러를 확인하라"를 **반증한다.**
> 실측이 우선이며, 해당 권고는 철회한다. 단 **일회성 읽기(`_sba_read`)는 핫루프가 아니므로
> 에러 검사를 유지**한다 — 두 경로를 혼동하지 말 것.
...
- 코어 전환은 **셋업 지점**이므로 버스트 경계에서만 일어난다 — 버스트 방식이 이 제약과
  자연스럽게 맞는다(샘플당 라운드로빈이었다면 매 샘플이 셋업이라 이 제약과 정면충돌).
- **호스트 부하 완화를 샘플 간 지연으로 할 수 없다.** 지연은 루프 밖(버스트 사이)에서만
  가능하므로, 부하 조절은 `sample_interval_us` 가 아니라 **버스트 길이·버스트 간 휴지**로 한다.
