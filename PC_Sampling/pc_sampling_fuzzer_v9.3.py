...
class NVMeFuzzer:
    ...
    def _llm_workload_feedback(self) -> str:
        """직전 버스트 결과(descriptor→FFM 델타)를 되먹임 문장으로. 없으면 빈 문자열."""
        r = self._last_workload_result
        if not r:
            return ""
        d = r.get('desc') or {}
        moved = (r.get('ffm_delta') or 0) > 0
        verdict = ("It advanced fragmentation/GC pressure — push further or vary parameters."
                   if moved else
                   "It did NOT move FFM — try a different pattern or parameters.")
        return (f"Feedback: your last workload {{pattern={r.get('pattern')}, "
                ...
                f"moved FFM {r.get('ffm_start')}→{r.get('ffm_peak')} (Δ{r.get('ffm_delta')}) "
    ...
    def _run_llm_workload_burst(self, desc: dict) -> None:
        ...
        self._last_workload_result = {
            ...
            'ffm_start': ffm0, 'ffm_peak': ffm_max, 'ffm_delta': ffm_delta,
            'stop': stop_reason,
        }
