...
> v9.0 = v8.4 전체 기능 보존(복사본) + **사내 RAG/LLM 기반 spec-aware 시드·시퀀스·분류** 추가.
> `--rag` off 면 v8.4와 byte-동등. v8.4 본체는 변경하지 않음(per-version 파일 규칙).

---

## v9.0 핵심: 사내 RAG/LLM 통합 (2노드, 비동기 drop-box)

NVMe base / 고객·vendor / TCG·ATA 스펙의 **산문(prose) 지식**을 활용해 ① 의미 있는 시드/시퀀스 생성
② 기존 시드 유의미성 평가·필터링 ③ 미접촉 명령군 개척 ④ coverage plateau 시 보강으로 성장 재가속.

- **2노드**: Test PC(NVMe+fuzzer, stdlib-only, LLM 미설치) ↔ Intranet PC(`rag_seed_service.py`, 사내 LLM
  HTTP REST + 로컬 임베딩) — **공유 FS drop-box**(`rag/{requests,inbox,sequences,archive,rejects,status}`)로만 통신.
- **비동기·비블로킹**: LLM은 fuzzer hot loop 밖. plateau/maintenance(1000-exec 주기) 때만 파일 I/O.
  LLM 장애 시 inbox만 비어 fuzzer 동작 불변.
- **fuzzer 측(이 파일)**: `--rag` 플래그 + config `rag` 섹션 + `RagBridge`(요청 emit/inbox·sequence 스캔/
  분류 적용/archive) + plateau 훅(maintenance 블록) + `Seed.seed_class` 필드(cull/energy/meta 훅) +
  skeleton→`BUILTIN_SEQUENCES` 흡수 + corpus snapshot export + dedup. 기존 외부시드 로더(`_load_seeds`) 재사용.
- **서비스 측(별도)**: ingest→embed(로컬, 교체가능)→vectordb→retrieve→`CMD_SCHEMAS` 기반 프롬프트→
  사내 LLM(REST)→`validate_and_repair`(동일 CMD_SCHEMAS로 정합성 검증, 위험명령 미생성)→inbox emit.
- **단계**: P0 공유 인프라(+mock LLM) → P1 시드 분류·필터링 → P2 시퀀스 skeleton → P3 신규 명령군 생성.
- 상세 설계: 세션 plan 문서 참조. **구현 진행 중**(현재 v9.0 = v8.4 복사 + 버전 표기).
