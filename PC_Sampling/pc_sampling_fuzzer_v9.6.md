...
```
[LLM-boost] 1.50 → 1.18  r=-0.714  y_llm=0.010000 y_mut=0.060000  exec=3000/500  sel=500/500  edge=30/30
[LLM-boost] 실행 표본 부족 — 갱신 보류, 누적 계속 (실행 llm=30, mutation=900, 필요=각 50)
[LLM-boost] 발견 부족 — 갱신 보류, 누적 계속 (edge 1+0=1, 필요=합계 5)
```
`exec` 와 `sel` 의 비(比)가 곧 **평균 시퀀스 길이**다. 위 예에서 LLM 은 500선택으로 3000실행
(=선택당 6명령)을 썼는데 mutation 과 발견 수가 같으므로 예산 효율이 1/6 이다.
```
```
