...
- **분모(기회 정규화)가 이 변경의 핵심.** v9.4 `_cov_by_src` 는 **발견 수만** 셌다. 그러면
  "많이 뽑혀서 많이 찾은 것" 과 "잘 찾은 것" 을 구분할 수 없다. 실행 수를 세야 yield 가 나온다.
...
| `_account_command`(`executions += 1` 직후) | **분모** — 발견 여부 무관 **모든 실행**을 origin(llm\|mutation)별로 카운트 |
...
```
[LLM-boost] 1.50 → 1.65  r=+0.333  y_llm=0.020000 y_mut=0.010000  exec=1000/9000  edge=20/90
[LLM-boost] 표본 부족 — 갱신 보류, 누적 계속 (llm=120, mutation=9880, 필요=200)
```
...
| `boost_max` | 3.0 | 상한 |
| `boost_min_samples` | 200 | 양 arm 각각의 최소 실행 수 |
...
- **arm 분리** — 현재 `llm_cmd`/`llm_seq` 가 하나의 boost 를 공유(비율만 유지). 표본이 충분해지면
  분리 가능하나, 나누면 각 arm 의 표본이 반으로 줄어 추정이 나빠진다.
