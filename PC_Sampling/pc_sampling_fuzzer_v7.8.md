# PC Sampling SSD Firmware Fuzzer v7.8

OpenOCD PCSR(PC Sampling Register) 비침습 샘플링 + `nvme-cli` passthru 기반 Coverage-Guided + State-Aware Fuzzer.

v7.8 핵심: **Unsupported-Command 자동 Skip & 복구 (EngineErrInt 기반)** — timeout crash 후 J-Link dump 를 customer parsing tool 로 분석, event log 의 `EngineErrInt` 토큰 검출 시 미지원 명령 으로 판정 → UFAS/artifact 건너뛰고 power cycle 후 메인 루프 계속. `--unsupported-skip` opt-in.

v7.7 의 PM Robustness Perturbation (S1+S2), v7.6 시각화 / v7.5 SequenceSeed corpus / ctx 모드 / 2-pass favored cull 모두 유지.
...
3. [v7.8 변경사항](#v78-변경사항)
4. [v7.7 변경사항](#v77-변경사항)
5. [v7.6 변경사항](#v76-변경사항)
...
### Unsupported-Command 자동 Skip & 복구 (EngineErrInt 기반)

펌웨어 업체가 "미지원 명령" 으로 dismiss 하는 timeout 을 자동 분리. timeout 후 J-Link dump 를 vendor parsing tool 로 분석해 event log 의 `EngineErrInt` 토큰을 ground truth 로 사용. 검출 시 UFAS dump / artifact 수집 건너뛰고 power cycle → 메인 루프 계속.

기본 OFF — `--unsupported-skip` 으로 opt-in.
...
`crashes/crash_<ts>/SKIPPED.marker`:
```
skipped_at  : 2026-05-26T14:30:52.123456
reason      : EngineErrInt detected in J-Link dump event log
command     : <cmd_name> (opcode=0x<NN>)
fuzz_data_md5: <hex>
```

→ audit 시 어느 명령이 언제 dismissable 로 분류됐는지 확인 가능.

#### Trade-off / 안전장치

- Parsing tool 없음 / timeout / non-zero rc → **False 반환 (보수적)** → 기존 timeout crash 흐름 진행. 의심스러우면 skip 안 함.
...
```
--unsupported-skip   timeout 후 J-Link dump 의 event log 에서 EngineErrInt 검출 시
                     미지원 명령으로 간주, power cycle + 메인 루프 계속.
```

미사용 시 기존 동작 100% 동일 (회귀 없음).
...
| 버전 | 주요 변경 |
|------|-----------|
| **v7.8** | Unsupported-Command 자동 Skip & 복구 (EngineErrInt 기반). timeout 후 J-Link dump 를 `customer_parsing_dump.py` 로 분석, `<dump>_customer_analysis/g16arEventLog*.txt` 의 `EngineErrInt` 검출 시 미지원 명령 으로 판정 → UFAS/artifact 건너뛰고 `_power_cycle_ssd` + `_por_pcie_rescan` + APST/KA disable + OpenOCD 재연결 → 메인 루프 계속. `crashes/crash_<ts>/SKIPPED.marker` 로 audit. 기본 OFF (`--unsupported-skip` opt-in). `--no-jlink` / 기존 OpenOCD-less 경로와 호환. `--no-jlink-dump` 시 자동 비활성. |
